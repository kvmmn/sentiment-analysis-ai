...
Open item for the team: `linkedin-scraper/linkedin_posts.csv` (197 rows, 4 September test run) is tracked in the public repository and contains post text, author names, profile URLs, and some emails in recruiter posts. It was committed knowingly as an engineering test artifact (sections 24–27, commit `4ed7a5e`). Removing it from the working tree is not enough to make it private; the history would also need rewriting, which changes shared commits. Decision left to Kaveh and the team; no change made here.

## Current desk (2026-09-10)

Iman package onboarded; feedback letter drafted for Kaveh to send; GitHub synchronized (`fbf0763`). Next: Kaveh reviews/sends the letter; Morteza’s check; a 2–3 vector pilot per the letter; team decision on the tracked test CSV. No harvest run here; no query freeze.
