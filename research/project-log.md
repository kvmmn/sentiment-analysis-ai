...
Open item for the team: `linkedin-scraper/linkedin_posts.csv` (197 rows, 4 September test run) is tracked in the public repository and contains post text, author names, profile URLs, and some emails in recruiter posts. It was committed knowingly as an engineering test artifact (sections 24–27, commit `4ed7a5e`). Removing it from the working tree is not enough to make it private; the history would also need rewriting, which changes shared commits.

**Decision (2026-09-10, Kaveh):** stop tracking the CSV. Done with `git rm --cached` (file kept on disk), `.gitignore` now excludes all `linkedin-scraper/*.csv` and `*.db`, and the scraper README marks the export as git-ignored. The file remains in earlier commits (`be67e98`, `4ed7a5e`); purging it would require rewriting `main` and a force push, which the working principles rule out. If the team wants the history cleaned, that is a separate, coordinated step (all clones re-cloned; GitHub support contacted for cached views).

## Current desk (2026-09-10)

Iman package onboarded; feedback letter drafted for Kaveh to send; GitHub synchronized (`fbf0763`). Next: Kaveh reviews/sends the letter; Morteza’s check; a 2–3 vector pilot per the letter; optional team decision on purging the test CSV from history. No harvest run here; no query freeze.
